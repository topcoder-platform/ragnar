...
task_def=$(printf "$task_template" $family $AWS_ACCOUNT_ID $AWS_ECS_CONTAINER_NAME $AWS_ACCOUNT_ID $AWS_REGION $AWS_REPOSITORY $TAG "$DEBUG" "$GITHUB_CLIENT_ID" "$GITHUB_CLIENT_SECRET" "$GITLAB_CLIENT_ID" "$GITLAB_CLIENT_SECRET" "$MONGODB_URI" "$NODE_ENV" "$NPM_CONFIG_PRODUCTION" "$PAPERTRAIL_API_TOKEN" "$TC_AUTHN_URL" "$TC_CLIENT_ID" "$WEBSITE" "$DISABLE_LOGGING" "$API_CONTEXT_PATH" $PORT $PORT $AWS_ECS_CLUSTER $AWS_REGION $ENV)
